...
export class UserService {
    ...
    static async createUser(
        ...
        passwordHash: string,
        role: string = 'student',
        details?: { fullName?: string; studentNumber?: string; jobNumber?: string }
    ): Promise<User> {
        const { fullName, studentNumber, jobNumber } = details || {};
        const result = await pool.query(
            `INSERT INTO users (username, password, role, full_name, student_number, job_number) 
             VALUES ($1, $2, $3, $4, $5, $6) 
             RETURNING id, username, role, full_name, created_at`,
            [username, passwordHash, role, fullName, studentNumber, jobNumber]
        );

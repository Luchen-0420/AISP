...
export class CaseGenerationService {
    ...
    static async generateVariant(
        templateId: string,
        params: { difficulty: string; compliance: string; personality: string; aim?: string },
        options?: { apiKey?: string; baseUrl?: string; modelName?: string }
    ) {
     ...
     */
    static async saveVariant(templateId: string, variantData: any) {

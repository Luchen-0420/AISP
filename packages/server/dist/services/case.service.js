...
export class CaseService {
    ...
    static async getCaseTemplates(teacherId) {
        const result = await pool.query('SELECT * FROM case_templates WHERE created_by = $1 ORDER BY created_at DESC', [teacherId]);
        return result.rows;
    }
    // Get a specific case template
    static async getCaseTemplateById(id) {
        const result = await pool.query('SELECT * FROM case_templates WHERE id = $1', [id]);
        return result.rows[0] || null;
    }
    // Get variants for a specific case template
    static async getVariantsByTemplateId(templateId) {
        const result = await pool.query('SELECT * FROM case_variants WHERE template_id = $1 ORDER BY created_at DESC', [templateId]);
        return result.rows;
    }
    ...
    static async createCaseTemplate(data) {
        ...
        const result = await pool.query(`INSERT INTO case_templates 
    ...
    static async deleteCaseTemplate(id) {

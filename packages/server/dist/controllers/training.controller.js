...
export class TrainingController {
    static async submitSession(req, res) {
        try {
            ...
            const { variantId, sessionId, scores, history, soapData, taskId, // Optional, might be null for free practice
    ...
    static async exportOSCE(req, res) {
        try {
            // @ts-ignore
            const studentId = req.user?.id;
            const { id } = req.params;
            ...
            const scores = session.final_score || {};
            const coverage = session.opqrst_coverage || {};
            // Generate CSV
            ...
            const totalBase = Object.values(scores).reduce((a, b) => a + b, 0);
            ...
            if (coverage.missed && Array.isArray(coverage.missed)) {
                coverage.missed.forEach((m) => csvContent += `- ${m}\n`);
            }
            else {
                csvContent += `None\n`;
            }

...
- **病历撰写**：聚焦 **病史采集 (S)** 与 **初步诊断 (A)** 的核心能力训练，去除冗余的处置计划环节。

#### 3. 智能反馈与推荐
- **个性化弱项分析**：系统自动分析历史训练数据，识别学生在“共情能力”或“临床逻辑”上的短板。
- **智能病例推荐**：基于弱项分析算法，自动推荐适合的针对性强化病例。
- **OSCE 报告导出**：支持一键导出包含雷达图得分、失分项详解的 CSV 格式成绩单。
...
### 1. "问了再说" (Ask Before Telling)
系统严格遵循启发式教学原则。在学生进行问诊开启前或过程中，AI 助教**不会**主动提供诊断线索或具体问题建议，而是通过评估学生的提问覆盖度，给予笼统的引导（如"请完善病史采集"），确保学生建立独立的临床思维，避免依赖性。

### 2. 核心流程闭环 (Core Closed Loop)
摒弃过度设计的辅助检查与复杂治疗方案，将训练重心聚焦于 **"病史采集 → 初步诊断"** 的黄金问诊闭环。通过减少非核心环节的干扰，高频强化医学生最基础也最重要的临床基本功。
